...
def _load_dinov2(model_name: str):
    ...
    When the environment variable ``DINOV2_LOCAL_PATH`` is set to a local
    clone of ``facebookresearch/dinov2`` the model is loaded offline via
    ``source='local'``; otherwise the default remote ``github`` source is used.
    """
    local_path = os.environ.get('DINOV2_LOCAL_PATH', '').strip()
    if not local_path:
        hub_fallback = os.path.expanduser(
            os.path.join('~', '.cache', 'torch', 'hub', 'facebookresearch_dinov2_main')
        )
        if os.path.isdir(hub_fallback):
            local_path = hub_fallback
    if local_path and os.path.isdir(local_path):
        return torch.hub.load(local_path, model_name, source='local', pretrained=True)
