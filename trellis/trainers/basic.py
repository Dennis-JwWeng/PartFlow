...
class BasicTrainer(Trainer):
    ...
    def load(self, load_dir, step=0):
        ...
        # Prefer LR from the current training config when resuming (chk stores optimizer LR too).
        if self.optimizer_config is not None and 'lr' in self.optimizer_config.get('args', {}):
            cfg_lr = float(self.optimizer_config['args']['lr'])
    ...
    def run_step(self, data_list):
        ...
        valid_data_list = [mb_data for mb_data in data_list if mb_data is not None]
        if len(valid_data_list) == 0:
            return None
        for i, mb_data in enumerate(valid_data_list):
            ## sync at the end of each batch split
            sync_contexts = [self.training_models[name].no_sync for name in self.training_models] if i != len(valid_data_list) - 1 and self.world_size > 1 else [nullcontext]
            with nested_contexts(*sync_contexts), elastic_controller_context():
                with amp_context():
                    loss, status = self.training_losses(**mb_data)
                    l = loss['loss'] / len(valid_data_list)
                ## backward
                if self.fp16_mode == 'amp':
