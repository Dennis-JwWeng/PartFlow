...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.initial_ddp_check = initial_ddp_check
        ...
        if self.world_size > 1 and self.initial_ddp_check:
            self.check_ddp()
        elif self.world_size > 1 and self.is_master:
            print('\nSkipping initial DDP check.')
    ...
    def _pre_init_models(self, **kwargs):
        """
        Hook called before init_models_and_more.
        Override in subclasses to load pretrained weights and freeze params
    ...
    def run(self):
        ...
        while self.step < self.max_steps:
            ...
            if hasattr(self.dataset, 'set_training_step'):
                self.dataset.set_training_step(self.step)
            if hasattr(self.dataset, 'maybe_rescan_missing'):
                self.dataset.maybe_rescan_missing(self.step)

            data_list = self.load_data()
            if data_list is None or len(data_list) == 0:
                if self.is_master:
                    print(f'[Trainer] skipped empty batch at step {self.step}', flush=True)
                continue
            step_log = self.run_step(data_list)
            if step_log is None:
                if self.is_master:
                    print(f'[Trainer] skipped step {self.step}: no valid micro-batch', flush=True)
                continue
            ...
            if self.is_master and self.step % self.i_print == 0:
                ...
                # Loss / grad only go to log.txt and TensorBoard by default; mirror scalars here for tail -f.
                if step_log is not None:
                    ld = step_log.get('loss')
                    if isinstance(ld, dict):
                        # Always print total `loss` (or fall back to mse) ...
                        for key in ('loss', 'mse'):
                            if key not in ld:
                                continue
                            try:
                                v = float(ld[key])
                                if np.isfinite(v):
                                    columns.append(f'{key}: {v:.4f}')
                            except (TypeError, ValueError):
                                pass
                            break
                        # ... and any of the render-loss diagnostics that are
                        # present (only emitted on steps where render loss ran).
                        for key in (
                            'fm_loss', 'render_loss', 'render_loss_weighted',
                            'render_lambda', 'render_to_fm_ratio',
                            'render_loss_mse', 'render_loss_dreamsim',
                            'render_n_used',
                            'loss_mask_keep', 'mask_keep_ratio',
                        ):
                            if key not in ld:
                                continue
                            try:
                                ...
                                if not np.isfinite(v):
                                    continue
                                if key == 'render_n_used':
                                    columns.append(f'{key}: {int(v)}')
                                elif key == 'render_to_fm_ratio':
                                    columns.append(f'{key}: {v:.3f}')
                                else:
                                    columns.append(f'{key}: {v:.4f}')
                            except (TypeError, ValueError):
                                pass
                    st = step_log.get('status')
                    if isinstance(st, dict) and 'grad_norm' in st:
                        try:
                            gn = float(st['grad_norm'])
                            if np.isfinite(gn):
                                columns.append(f'grad_norm: {gn:.4f}')
                        except (TypeError, ValueError):
                            pass
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ...
                    # show with mlflow
                    # Some log leaves are strings/dicts (e.g. wall clock text), so
                    # guard isnan checks to numeric scalars only.
                    def _is_nan_scalar(x):
                        if isinstance(x, (np.floating, np.integer, float, int)):
                            return bool(np.isnan(x))
                        return False

                    log_show = [l for _, l in log if not dict_any(l, _is_nan_scalar)]

                    def _mean_numeric(values):
                        nums = [float(v) for v in values if isinstance(v, (np.floating, np.integer, float, int))]
                        if len(nums) == 0:
                            return np.nan
                        return float(np.mean(nums))

                    log_show = dict_reduce(log_show, _mean_numeric)
                    log_show = dict_flatten(log_show, sep='/')
                    for key, value in log_show.items():
                        if isinstance(value, (np.floating, np.integer, float, int)) and not np.isnan(value):
